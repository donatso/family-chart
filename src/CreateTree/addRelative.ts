...
activate(datum: TreePerson) {
  if (this.is_active) this.onCancel?.()
  this.is_active = true

  const store = this.store

  this.store_data = store.state
  this.datum = datum
  datum = JSON.parse(JSON.stringify(this.datum))

  const datum_rels = getDatumRelsData(datum, this.getStoreData()!.data, this.addRelLabels)
  ...
  function onChange(updated_datum: TreePerson) {
...
setAddRelLabels(add_rel_labels:Partial<Record<'father' | 'mother' | 'spouse' | 'son' | 'daughter',string>>) {
  ...
  for (let key in add_rel_labels) {
    const _key = key as keyof (typeof add_rel_labels)
    this.addRelLabels[_key] = add_rel_labels[_key]
  }
  return this
}
...
function getDatumRelsData(datum: TreePerson, store_data:TreePerson[], addRelLabels: Partial<Record<'father' | 'mother' | 'spouse' | 'son' | 'daughter',string>>) {
...
function findRel(store_data: TreePerson[], id: string) {
  return JSON.parse(JSON.stringify(store_data.find(d => d.id === id)))
}

function getDatumRels(datum: TreePerson, data:TreePerson[]) {
  const datum_rels = [datum]
  Object.keys(datum.rels).forEach((rel_type) => {
    const rel = datum.rels[rel_type as keyof TreePerson['rels']]
    if (Array.isArray(rel)) {
      rel.forEach(rel_id => {
        findAndPushRel(rel_type as keyof TreePerson['rels'], rel_id)
      })
    } else if(rel !== undefined) {
      findAndPushRel(rel_type as keyof TreePerson['rels'], rel)
    }
  })
  return datum_rels

  function findAndPushRel(rel_type: keyof TreePerson['rels'], rel_id: string) {
