...
export default function AddRelativeTree({cont, d_id, store, card_edit, card_dim, transition_time}) {
  const datum = store.getData().find(d => d.id === d_id),
    tree = CalculateTree({datum, data_stash: store.getData(), card_dim}),
    view = View({cont, tree, datum, store, card_edit, card_dim})
  ...
  cont.appendChild(div_add_relative)
